...
const WrapperSubMenu = ({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      refTimeOut.current = setTimeout(() => {
        const listSub = refWrapperListSubMenu?.current?.querySelectorAll(".item-sub-navigation");

        if (listSub?.length) {
          let mWidth = 0;
          listSub.forEach(element => {
            const offsetWidth = element?.offsetWidth + 28;
            mWidth += offsetWidth;
          });
  ...
  const handleScroll = type => {
    const wrapper = refWrapperListSubMenu?.current;
  ...
  return /*#__PURE__*/_jsxs(Box, {
    ...
    children: [isDesktopDown && /*#__PURE__*/_jsxs(Box, {
      ...
      onClick: () => onBuy?.(itemSelect.value),
      children: titleBtn
    })]
  });
};
