...
const FileUploader = props => {
  ...
  const TEXT_ERROR = {
    maxSize: errorText ?? `Keep your file within ${maxSize} MB`,
    wrongFormat: wrongFormat ?? "Upload a PDF, PNG or JPG file",
    limitFiles: limitFiles ?? 'Upload up to 5 files'
  };
  ...
  function dropHandler(ev) {
    const length = ev.dataTransfer.items?.length || 0; // Prevent default behavior (Prevent file from being opened)
