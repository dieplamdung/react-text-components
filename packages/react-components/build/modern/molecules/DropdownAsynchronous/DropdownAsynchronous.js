...
const DropdownAsynchronous = ({
  ...
  const handleBlur = () => {
    ...
    if (isMin) {
      if (!valueSelect) {
        ...
        onChange?.(refValueSelect.current);
        onChangeCountry?.(refCountryDefault?.current);
        return;
      }

      refValueSelect.current = valueSelect;
      refCountryDefault.current = countrySelect?.country;
    }
  };
  ...
  const filterOptions = (ops, state) => {
    const newOptions = [];
    ops.forEach(option => {
      if (option?.label?.toLowerCase().includes(state?.inputValue?.toLowerCase()) || isCountryCode && option?.country?.toLowerCase().includes(state?.inputValue?.toLowerCase())) {
    ...
    children: [/*#__PURE__*/_jsxs(Box, {
      ...
      children: [/*#__PURE__*/_jsx(Autocomplete, _extends({
        ...
        renderOption: (prs, option) => /*#__PURE__*/_jsx("li", _extends({}, prs, {
          style: {
            background: isCountryCode ? `${JSON.stringify(countrySelect) === JSON.stringify(option) ? "rgb(221 221 221 / 80%)" : ""}` : `${option?.label === valueSelect ? "rgb(221 221 221 / 80%)" : ""}`,
            pointerEvents: "auto"
          },
          children: /*#__PURE__*/_jsxs(Box, {
            sx: {
